...
// Bandlardagi `icon` — `icons.js` dagi ikonka NOMI, chizish `linkHtml` da.
const MAIN_LINKS = [
  { href: ROUTES.home, icon: "grid", key: "nav.home" },
  { href: ROUTES.restaurants, icon: "restaurant", key: "nav.restaurants" },
  { href: ROUTES.venues, icon: "party", key: "nav.venues" },
];
...
function linkHtml({ href, icon: name, key }) {
  return `
...
function tabbarItems(user) {
  const base = [
    { href: ROUTES.home, icon: "grid", key: "nav.shortHome" },
    { href: ROUTES.restaurants, icon: "restaurant", key: "nav.shortRestaurants" },
    { href: ROUTES.venues, icon: "party", key: "nav.shortVenues" },
  ];
  ...
  if (!user) {
    return [...base, { href: ROUTES.login, icon: "user", key: "nav.shortLogin" }];
  }
  ...
  if (user.is_staff) {
    return [
      ...base,
      { href: ROUTES.adminHome, icon: "shield", key: "nav.shortAdmin" },
      { href: ROUTES.profile, icon: "user", key: "nav.shortProfile" },
    ];
  }

  return [
    ...base,
    { href: ROUTES.myBookings, icon: "calendar", key: "nav.shortBookings" },
    { href: ROUTES.profile, icon: "user", key: "nav.shortProfile" },
  ];
}
...
function renderTabbar(user) {
  ...
  bar.innerHTML = tabbarItems(user).map(({ href, icon: name, key, action }) => {
    const inner = `
      <span class="ic">${icon(name)}</span>
...
export function initPublicNav() {
  ...
  const secondary = !user
    ? []
    : user.is_staff
      ? [{ href: ROUTES.profile, icon: "user", key: "nav.profile" }]
      : [
          { href: ROUTES.myBookings, icon: "calendar", key: "nav.bookings" },
          { href: ROUTES.profile, icon: "user", key: "nav.profile" },
        ];
  ...
  if (user?.is_staff) {
    secondary.push({ href: ROUTES.adminHome, icon: "shield", key: "nav.admin" });

...
// Yozuv turiga qarab ikonka. Sharh yulduzi to'ldirilgan chiqadi —
// reyting belgisi bo'sh kontur bilan tanilmaydi.
const ICONS = {
  reservation: { name: "calendar" },
  application: { name: "document" },
  subscription: { name: "gem" },
  review: { name: "star", fill: true },
  system: { name: "grid" },
};
...
function rowHtml(item) {
  const mark = ICONS[item.kind] || ICONS.system;
  const inner = `
    <span class="ic ${esc(item.level)}">${icon(mark.name, { fill: mark.fill })}</span>
    <span class="text">
      <b>${esc(item.title)}</b>

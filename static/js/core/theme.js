...
export function themeToggleHtml() {
  ...
  return `
    <button class="icon-btn" type="button" data-theme-toggle
      ...
      <span>${isDark ? icon("sun") : icon("moon")}</span>
    </button>`;
}
...
export function bindThemeToggle(root = document) {
  root.querySelectorAll("[data-theme-toggle]").forEach((button) => {
    button.addEventListener("click", () => {
      theme.toggle();
      const isDark = effective() === "dark";
      // Ikonka SVG matni bo'lgani uchun `textContent` emas, `innerHTML`.
      button.querySelector("span").innerHTML = isDark ? icon("sun") : icon("moon");

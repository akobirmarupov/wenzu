  ...
  return `
    <div class="list-row">
      ...
      <div class="list-row-actions">
        ...
        <button class="btn btn-sm btn-danger" data-delete-banner="${esc(banner.id)}" aria-label="O'chirish">${icon("trash")}</button>
      </div>
    </div>`;
}
...
async function loadBanners() {
  ...
  try {
    ...
    container.innerHTML = banners.length
      ...
      : emptyState("Banner yo'q", "Bosh sahifada banner ko'rinishi uchun bittasini qo'shing.", icon("megaphone"));
  } catch (error) {
...
function newsRow(item) {
  return `
    <div class="list-row">
      <div class="stack stack-1" style="min-width:240px">
        <b>${item.is_pinned ? `${icon("mapPin")} ` : ""}${esc(item.title_uz)}</b>
      ...
      <div class="list-row-actions">
        ...
        <button class="btn btn-sm btn-danger" data-delete-news="${esc(item.id)}" aria-label="O'chirish">${icon("trash")}</button>
      </div>
    </div>`;
}
    ...
    <form class="stack stack-4" id="news-form" style="margin-top:var(--sp-5)">
      ...
      <div class="row row-4 row-wrap">
        ...
        <label class="checkbox">
          ...
          <span>${icon("mapPin")} Mahkamlash (birinchi turadi)</span>
        </label>
      </div>
...
async function loadNews() {
  ...
  try {
    ...
    container.innerHTML = newsItems.length
      ...
      : emptyState("Yangilik yo'q", "Bosh sahifadagi lenta bo'sh qolmasligi uchun qo'shing.", icon("news"));
  } catch (error) {
    container.innerHTML = errorState(error.message);
  }
}

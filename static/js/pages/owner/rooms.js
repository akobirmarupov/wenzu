    ...
    <div class="card">
      ...
      <div class="card-body">
        ...
        <div class="row row-2" style="margin-top:var(--sp-2)">
          ...
          <button class="btn btn-sm btn-danger" data-delete="${esc(room.id)}" aria-label="O'chirish">${icon("trash")}</button>
        </div>
      </div>
    </div>`;
}
...
async function load() {
  ...
  try {
    ...
    $("#list").innerHTML = rooms.length
      ? rooms.map(card).join("")
      : emptyState("Xonalar yo'q", "Birinchi xonangizni qo'shing — mijozlar shuni bron qiladi.", icon("seat"));
  } catch (error) {

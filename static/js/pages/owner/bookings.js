  ...
  return `
    <div class="list-row">
      <div class="stack stack-2" style="min-width:260px">
        ...
        ${menu ? `<span class="xs faint">${icon("restaurant")} ${esc(menu)}</span>` : ""}
        ${reservation.special_request ? `<span class="xs faint">${icon("chat")} ${esc(reservation.special_request)}</span>` : ""}
      </div>
...
async function load() {
  ...
  try {
    ...
    $("#list").innerHTML = data.results.length
      ...
      : emptyState("Bronlar yo'q", "Bu filtr bo'yicha hech narsa topilmadi.", icon("calendar"));

    ...
    <div class="panel" style="margin-top:var(--sp-5)">
      ...
      <div class="row row-2 row-wrap" style="margin-top:var(--sp-5)">
        <a class="btn btn-outline" href="https://t.me/${esc(telegram.replace("@", ""))}"
           target="_blank" rel="noopener">${icon("send")} ${esc(telegram)} bilan bog'lanish</a>
      </div>
    </div>`);
...
function renderPayments(payments) {
  $("#payments").innerHTML = payments.length
    ...
    : emptyState("To'lovlar yo'q", "Hali to'lov tasdiqlanmagan.", icon("receipt"));
}
...
async function renderRequests() {
  ...
  try {
    ...
    container.innerHTML = data.results.length
      ...
      : emptyState("Ariza yuborilmagan", "Tarifni tanlasangiz shu yerda ko'rinadi.", icon("mail"));
  } catch (error) {

...
/* `suffix` — faqat ikonka HTML'i qo'yiladi, foydalanuvchi matni emas. */
function statCard(label, value, { accent = false, hint = "", suffix = "" } = {}) {
  return `
    <div class="stat-card">
      <span class="label">${esc(label)}</span>
      <span class="value ${accent ? "accent" : ""}">${esc(String(value))}${suffix}</span>
...
function quickLinks() {
  const place = isVenue
    ? { href: "/panel/zallar/", icon: "venue", title: "Zallar", text: "Sig'im va depozit" }
    : { href: "/panel/xonalar/", icon: "seat", title: "Xonalar", text: "Stol va VIP xonalar" };

  return [
    { href: "/panel/bronlar/", icon: "calendar", title: "Bronlar", text: "Tasdiqlash va bekor qilish" },
    place,
    { href: "/panel/menyu/", icon: "restaurant", title: "Menyu", text: "Taom, narx va rasm" },
    { href: "/panel/jadval/", icon: "calendarDays", title: "Bo'sh vaqtlar", text: "Qaysi kunlar ochiq" },
    { href: "/panel/sharhlar/", icon: "star", fill: true, title: "Sharhlar", text: "Mijozlar nima deydi" },
    { href: "/panel/sozlamalar/", icon: "settings", title: "Sozlamalar", text: "Ma'lumot va galereya" },
  ];
}
...
function init() {
  render("#quick-links", quickLinks().map((link) => `
    <a class="card card-link shortcut-card" href="${link.href}">
      <span class="ic">${icon(link.icon, { fill: link.fill })}</span>
      <b>${esc(link.title)}</b>
  ...
  try {
    ...
    render("#stats", [
      ...
      statCard("Reyting", Number(stats.rating_avg || 0).toFixed(1),
        { suffix: ` ${icon("star", { fill: true, className: "ico-gold" })}`, hint: `${stats.reviews_count} sharh` }),
    ...
    $("#recent").innerHTML = data.recent_reservations.length
      ...
      : emptyState("Hozircha bronlar yo'q", "Mijozlar sizni bron qilganda shu yerda ko'rinadi.", icon("calendar"));
    ...
    render("#business-card", `
      <div class="panel-head">
        ...
        <div class="row row-2">
          ...
          <a class="btn btn-ghost btn-sm" href="/panel/sozlamalar/">Tahrirlash ${icon("chevronRight")}</a>
        </div>
      </div>
      ...
      <a class="btn btn-outline btn-sm" style="align-self:flex-start;margin-top:var(--sp-4)"
         href="/biznes/${esc(business.id)}/">Mijoz ko'zi bilan ko'rish ${icon("chevronRight")}</a>
    `);
  } catch (error) {
    render("#stats", "");

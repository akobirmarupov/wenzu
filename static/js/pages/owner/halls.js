    ...
    <div class="card">
      ...
      <div class="card-body">
        ...
        <div class="row row-2" style="margin-top:var(--sp-2)">
          ...
          <button class="btn btn-sm btn-danger" data-delete="${esc(hall.id)}" aria-label="O'chirish">${icon("trash")}</button>
        </div>
      </div>
    </div>`;
}
...
async function load() {
  ...
  try {
    ...
    $("#list").innerHTML = halls.length
      ? halls.map(card).join("")
      : emptyState("Zallar yo'q", "Birinchi zalingizni qo'shing.", icon("venue"));
  } catch (error) {

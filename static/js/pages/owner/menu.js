    ...
    <div class="menu-item">
      ...
      <div class="body">
        ...
        <div class="row row-2" style="margin-top:var(--sp-2)">
          <button class="btn btn-sm btn-outline" style="flex:1" data-edit="${esc(item.id)}" aria-label="Tahrirlash">${icon("edit")}</button>
          <button class="btn btn-sm btn-danger" data-delete="${esc(item.id)}" aria-label="O'chirish">${icon("trash")}</button>
        </div>
      </div>
    </div>`;
}
...
async function load() {
  ...
  try {
    ...
    $("#list").innerHTML = items.length
      ? items.map(card).join("")
      : emptyState("Menyu bo'sh", "Birinchi taomingizni qo'shing.", icon("restaurant"));
  } catch (error) {
